...
describe("getApiRequestTimeout", () => {
	...
	it("should return default timeout of 3600000ms when no configuration is set", () => {
		...
		expect(timeout).toBe(3600000) // 3600 seconds in milliseconds (1 hour)
	})
	...
	it("should handle null by using default", () => {
		...
		expect(timeout).toBe(3600000) // Should fall back to default 3600 seconds (1 hour)
	})
	...
	it("should handle string values by using default", () => {
		...
		expect(timeout).toBe(3600000) // Should fall back to default since it's not a number
	})
