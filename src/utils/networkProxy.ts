...
let globalUndiciDispatcherSet = false
...
export async function initializeNetworkProxy(
	...
	// Set up the undici global dispatcher with the user's configured timeout.
	// This must run in ALL modes (not just debug) because Node.js's built-in fetch()
	// and packages like the ollama npm client are subject to undici's default 300s timeout.
	await initializeGlobalUndiciDispatcher()
...
/**
 * Initialize the undici global dispatcher with the user's configured API request timeout.
 *
 * Node.js's built-in `fetch()` (Node 18+) is powered by an internal copy of undici that
 * defaults to a 300-second connect/headers/body timeout. Packages like the ollama npm client
 * use `fetch()` internally and will time out after 300s regardless of the user's
 * `apiRequestTimeout` setting.
 *
 * By setting an undici `Agent` as the global dispatcher with the user's configured timeout,
 * all `fetch()`-based requests (including those from the ollama package) will respect the
 * `roo-cline.apiRequestTimeout` VS Code setting.
 */
async function initializeGlobalUndiciDispatcher(): Promise<void> {
	if (globalUndiciDispatcherSet) {
		return
	}

	try {
		const { Agent, setGlobalDispatcher } = (await import("undici")) as typeof import("undici")

		const timeoutMs = getApiRequestTimeout()

		// If the user explicitly set timeout to 0 (disabled), don't override the default dispatcher.
		...
		globalUndiciDispatcherSet = true
		log(`undici global dispatcher configured with timeout: ${timeoutMs}ms (${timeoutMs / 1000}s)`)
	} catch (error) {
		log(
			`Failed to configure undici global dispatcher: ${error instanceof Error ? error.message : String(error)}`,
		)
	}
}
